/* objective
 *      C = A*B_T  // A[m][k], B[n][k], C[m][n]
 * compile: nvcc -03 matmul_double_t.cpp kernel_nt.cu -o matmul_double_t
 */
...
void init_t(double *A, double *B, int M , int N, int K)
{
    for (int i = 0; i < M; ++i)
    {
        for (int j = 0; j < K; ++j)
...
void matmul_double_host_t(double* A, double* B, double* C, int M, int N, int K)
{
    for (int i = 0; i < M; ++i)
    {
        for (int j = 0; j < N; ++j)
...
int main(int argc, char *argv[])
{
    ...
    /* host compute */
    matmul_double_host_t(hA, hB, hC, M, N, K);
   
    /* device compute wrapper. Defined in matmul_double_t.cu */
    matmul_double_device_t(hA, hB, hC, M, N, K, dtohC);
